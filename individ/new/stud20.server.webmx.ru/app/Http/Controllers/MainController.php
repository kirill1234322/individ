...
class MainController extends Controller
{
	...
	public function acblog () {
		...
		return view("blog")->with(["data" => $data, "attachdata" => $attachdata, "menu_data" => $menu_data]);
	}
	...
	public function acNovosti () {
		...
		// по идентификатору главной получим прикрепленные записи 
		$attachdata = DB::table("posts")->where("parent", "=", $datas->id)->get(); 
		// отдадим полученные данные в представление
		
		return view("novosti")->with(["data" => $data, "attachdata" => $attachdata, "menu_data" => $menu_data]);
	}
	...
	public function acHistory () {
		...
		// отдадим полученные данные в представление
		return view ("history")->with(["data" => $data, "menu_data" => $menu_data]);
	}
	...
	public function acSubhistory ($subzhivopis) {
		...
		$menu_data = DB::table("posts")->get();
		// отдадим полученные данные в представление
		return view ("history")->with(["data" => $data, "attachdata" => $attachdata, "menu_data" => $menu_data]);
	}
	...
	public function acOpportunities () {
		...
		// отдадим полученные данные в представление
		return view("opportunities")->with(["data" => $data, "attachdata" => $attachdata, "menu_data" => $menu_data]);
	}
	...
	public function acBrowser () {
		...
		$attachdata = DB::table("posts")->where("parent", "=", $data->id)->get(); 
		// отдадим полученные данные в представление
		return view("browser")->with(["data" => $data, "attachdata" => $attachdata, "menu_data" => $menu_data]);
	}
	...
	public function acSubGoroda ($subgorod) {
		...
		$attachdata = DB::table("posts")->where("parent", "=", $data->id)->get(); 
		// отдадим полученные данные в представление
		return view ("goroda")->with(["data" => $data, "attachdata" => $attachdata, "menu_data" => $menu_data]);
	}	
	...
	public function acNeobychnyeMuzei () {
		...
		// отдадим полученные данные в представление
		return view ("neobychnye-muzei-mira")->with(["data" => $data, "attachdata" => $attachdata, "menu_data" => $menu_data]);
		}
